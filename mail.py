...
def sendMailLinux(fromEmail,fromPassword,subject,contentPath,contentFilename,maillistPath,maillistFilename,attachmentPath,attachmentFilename):
    # create message object instance
    msg = MIMEMultipart()
    # setup the parameters of the message
    password = fromPassword
    ...
    # Login Credentials for sending the mail
    server.login(msg['From'], password)
    for mail in getMailListLinux(maillistPath,maillistFilename):
        # send the message via the server.
        server.sendmail(msg['From'], mail, msg.as_string())
    #print(message)
    server.quit()
...
def getMessageLinux(path,filename):
    dataFolder = Path(path)
    file = dataFolder/filename
    f = open(str(file), "r")
    return f.read()

def getMailListLinux(path,filename):
    dataFolder = Path(path)
    file = dataFolder/filename
    f = open(str(file), "r")
    return f.read().strip().split(',')
